...
bool extract_embedded(std::wstring& out_path) {
    DWORD size = 0;
    const void* data = find_rcdata(g_self_module, IDR_DANNYHACK_JAR, size);
    if (!data || size == 0) {
        log::error("PE resource lookup for IDR_DANNYHACK_JAR (RT_RCDATA) failed");
        return false;
    }
    ...
    std::swprintf(path, MAX_PATH, L"%sdannyhack-%lu.jar", tmp, GetCurrentProcessId());
    ...
    log::info("Extracted dannyhack.jar (%lu bytes) to %ls", (unsigned long)size, path);
    return true;
}
