...
DWORD WINAPI inject_thread(LPVOID) {
    using namespace dannyhack;

    log::init();
    log::info("dannyhack.dll bootstrap thread started, pid=%lu", GetCurrentProcessId());
    ...
    args.name = const_cast<char*>("dannyhack-Bootstrap");
    args.group = nullptr;
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        dannyhack::g_self_module = module;
