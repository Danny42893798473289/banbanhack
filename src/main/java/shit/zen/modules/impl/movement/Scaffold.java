...
public class Scaffold extends Module {
    ...
    private Rotation getTargetRotation(boolean firstGround) {
        ...
        if (this.groundTicks > 0) {
            ...
            // 加上人手甩鼠标时随机的力度误差
            // RandomUtils.nextFloat requires both bounds >= 0 — use offset form.
            clampLimit += -5.0f + RandomUtils.nextFloat(0.0f, 20.0f);
    ...
    private Rotation getSnappedRotation(Rotation rotation) {
        ...
        if (deltaYaw > 45.0) {
            // RandomUtils.nextFloat requires both bounds >= 0 — use offset form.
            noiseYaw += -1.5f + RandomUtils.nextFloat(0.0f, 3.0f); // 甩枪时的误差补偿
            noisePitch += -0.8f + RandomUtils.nextFloat(0.0f, 1.6f);
        }
